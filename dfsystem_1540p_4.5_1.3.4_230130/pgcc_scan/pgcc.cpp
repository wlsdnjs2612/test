...
void trim(char *s, unsigned int len)
{
	unsigned int i, j;
	char buf[len + 1];
	...
	memcpy(s, buf, len * sizeof(char));
}


int scanname_save(const char *buf)
{
	string source;
	char *s, *p;
	int n = 0;

	char check_str[2048] = { 0 };
	char data[9000][32];
	char *o;

	while ((buf = strstr(buf, "set_scan"))) {
		if ((s = strchr(buf, '('))) {
			buf = s;
			source = buf;

			if ((p = strtok((char*)source.c_str(), "\n")) && strstr(p, ";")) {
				strcpy( check_str, p );
				o = strtok( check_str, ")" ); 
				strcpy( data[n], o+1 );
				trim( data[n], strlen(data[n]) );
				scan_str.push_back(data[n]);
				n++;
			}
		}
	}
...
/*	for( int i = 0 ; i < n ; i++ ) {
		printf("data : %s size : %d\n", scan_str[i], strlen( scan_str[i] )  );
	}*/
	
	return 0;
}

bool scanfunction_check(const char *buf)
{
	char *p, *o, *source;
	int n = 0;
	...
	for (;;) {
		...
		if ( strncmp(source, "set_scan", strlen("set_scan")) != 0 ) {
			continue;
		}

		source = source + strlen("set_scan");
		p = strtok(source, "\n" );
		...
		while( p != NULL ) {
			...
			if( strstr( p, "!" )) {
				scanfunction_str1.push_back("false");
			}

			if( strstr( p, "==" )) {
				scanfunction_str.push_back( strstr(p,"==") );
			}
		
			if( strstr( p, "parseCommand" ) ) {
				break;
			}
		}
		
		char save_data[scanfunction_str.size()][100];
		
		for( int i = 0 ; i < (int)scanfunction_str.size(); i++) {
			strcpy( save_data[i],  scanfunction_str[i] );
			o = strtok( save_data[i], ")" );
			strcpy( save_data[i], o+2 );
			trim( save_data[i], strlen(save_data[i]) + 1 );
			n++;
		}
		
		for( int i = 0 ; i < n; i++) {
			scanfunction_str1.push_back(save_data[i]);
		}

/*		printf("-----------------------------------------------------------------------------------------\n");
		printf("1. size : %d\n", (int)scanfunction_str1.size() );
		for( int i = 0 ; i < (int)scanfunction_str1.size(); i++) {
			printf("scan() vector data : %s len : %d\n", scanfunction_str1[i], strlen(scanfunction_str1[i]) );
		}

		printf("2. size : %d\n", (int)scan_str.size() );
		for( int i = 0 ; i < (int)scan_str.size() ; i++ ) {
			printf("data : %s size : %d\n", scan_str[i], strlen( scan_str[i] )  );
		}
		printf("-----------------------------------------------------------------------------------------\n");
*/
		bool state = false;
		char interlock_name[100];

		for( int i = 0 ; i < (int)scan_str.size() ; i++ ) {
			for( int j = 0 ; j < (int)scanfunction_str1.size(); j++) {
				if( 0 == strcmp( scan_str[i], scanfunction_str1[j] )) {
					state = false;
					break;
				}
				else {
					state = true;
					strcpy( interlock_name, scan_str[i] );
				}
			}
			
			if( state == true ) {
				printf(" scan Function Name Error : %s / CHECK PLEASE!!!\n", interlock_name );
				return false;
			}
		}
...
//	char *source;
	...
	file_buffer[size] = 0;
	strFile = file_buffer;  // pgm source
	src_org = (char *) strFile.c_str();
	eatComment(file_buffer);  //주석 부분 제거
	strPattern = file_buffer;
	strTimeset = file_buffer;
	strScanCheck = file_buffer;
//	source = (char *) strFile.c_str();
	delete[] file_buffer;
	close(fd);
...
//	if (TimesetCompile(strTimeset.c_str()) == false || PatternCompile(strPattern.c_str()) == false) {
	...
	scanname_save(strScanCheck.c_str());
	if( scanfunction_check(strScanCheck.c_str()) == false) {
		return 1;
	}
